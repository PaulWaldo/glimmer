...
func NewPhotoCard(title, subtitle, photoUrl string) *PhotoCard {
	...
	card.Card = widget.NewCard(title, subtitle, canvas.NewRectangle(color.Black))
...
func (c *PhotoCard) loadImage() {
	// Simulate a really long download
	time.Sleep(time.Second * time.Duration(1))
	uri, err := storage.ParseURI(c.photoUrl)
	if err != nil {
...
func main() {
	...
	for i := 0; i < len(images); i++ {
		card := NewPhotoCard("Image Loading Tester", "This is a test to see if the image loading is working.", images[i])
